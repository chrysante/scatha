...
TEST_CASE("SymbolTable lookup", "[sema]") {
    ...
    auto const var = sym.addVariable(Token("x"), sym.Int());
...
TEST_CASE("SymbolTable define custom type", "[sema]") {
    ...
    auto const fnI = sym.declareFunction(Token("i"));
    ...
    auto& xType = sym.declareObjectType(Token("X")).value();
    // Begin X
    sym.pushScope(xType.symbolID());
    // Add member variable 'i' to
    auto const memberI = sym.addVariable(Token("i"), sym.Int());
    sym.popScope();
    // End X
    xType.setSize(8);
