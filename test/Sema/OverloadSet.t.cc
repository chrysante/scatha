...
TEST_CASE("OverloadSet") {
    ...
    auto f_int = sym.addFunction(Token("f"), sema::FunctionSignature({ sym.Int() }, sym.Int()));
    CHECK(f_int.hasValue());

    // Declare a function f: (float) -> float
    auto f_float = sym.addFunction(Token("f"), sema::FunctionSignature({ sym.Float() }, sym.Float()));
    CHECK(f_float.hasValue());

    // Declare a function f: (float) -> int
    auto f_float2 = sym.addFunction(Token("f"), sema::FunctionSignature({ sym.Float() }, sym.Int()));
    ...
    // Declare a function f: (float) -> float
    auto f_float3 = sym.addFunction(Token("f"), sema::FunctionSignature({ sym.Float() }, sym.Float()));
