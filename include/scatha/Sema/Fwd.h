...
SC_ENUM_SIZE_LAST_DEF(scatha::sema::SpecialMemberFunctionDepr, Delete);

namespace scatha::sema {

SCATHA_API std::string_view toString(SpecialMemberFunctionDepr);

SCATHA_API std::ostream& operator<<(std::ostream&, SpecialMemberFunctionDepr);
...
enum class SpecialLifetimeFunctionDepr : uint8_t {
#define SC_SEMA_SPECIAL_LIFETIME_FUNCTION_DEF(Name) Name,
#include <scatha/Sema/Lists.def>
};

/// Get the corresponding special member function (`new`, `move` or `delete`)
SCATHA_API SpecialMemberFunctionDepr toSMF(SpecialLifetimeFunctionDepr);

} // namespace scatha::sema

SC_ENUM_SIZE_LAST_DEF(scatha::sema::SpecialLifetimeFunctionDepr, Destructor);
