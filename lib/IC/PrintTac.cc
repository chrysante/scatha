...
void printTac(ThreeAddressCode const &tac, sema::SymbolTable const &sym) { printTac(tac, sym, std::cout); }
                                    ...
                                    for (auto const &line : tac.statements) {
                                        std::visit(utl::visitor{[&](Label const &label) {
                                                                ...
                                                                [&](FunctionLabel const &label) {
                                                                    auto const &function =
                                                                        sym.getFunction(label.functionID());
                                                                    str << function.name() << ":\n";
                                                                },
                                                                ...
                                                                [&](ThreeAddressStatement const &s) {
                                                                    ...
                                                                    if (s.result.is(TasArgument::conditional)) {
                                                                        if (s.operation == Operation::ifPlaceholder) {
                                                                            ...
                                                                            SC_ASSERT(isRelop(s.operation), "Must be "
                                                                                                            "relop");
                                                                            str << "if " << s.operation << " "
                                                                                << print(s.arg1, sym) << ", "
                                                                                << print(s.arg2, sym) << "\n";
