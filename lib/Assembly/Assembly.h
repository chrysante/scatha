...
namespace scatha::assembly {
	...
	/**
	 The same instruction set as enum class OpCode in VM/OpCcode.h, but with a kind of overload resolution,
	 meaning the exact opcode will be deduced from the arguments by the assembler.
	 E.g. 'movRR', 'movRV', 'movMR', 'movRM' are here only represented as 'mov'.
	 */
	...
	struct Value64 {
		...
		explicit Value64(u64 value, Type type = UnsignedIntegral):
			type(type),
			value(value)
		{}
	...
	enum class Marker: u16 {
		...
		// End of program. Must not be written into the assembler, only used internally.
		EndOfProgram  = 1 << 8,
	};
