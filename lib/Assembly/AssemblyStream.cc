...
std::span<std::string const> AssemblyStream::foreignLibraries() const {
    return impl->foreignLibs;
}

void AssemblyStream::setForeignLibraries(std::vector<std::string> libs) {
    impl->foreignLibs = std::move(libs);
}
