...
namespace scatha::vm {
	...
	static auto jump(auto cond) {
		return [](u8 const* i, u64*, VirtualMachine* vm) -> u64 {
			...
			if (decltype(cond)()(vm->flags)) {
				vm->iptr = utl::bit_cast<i64>(vm->iptr) + offset;
				return 0;
			}
			return ijmp(C);
		};
	...
	utl::vector<Instruction> makeInstructionTable() {
		...
		/// MARK: Jumps
		at(jmp) = jump<jmp>([](VirtualMachine::Flags f) { (void)f; return true; });
		at(je)  = jump<je> ([](VirtualMachine::Flags f) { return f.equal; });
		at(jne) = jump<jne>([](VirtualMachine::Flags f) { return !f.equal; });
		at(jl)  = jump<jl> ([](VirtualMachine::Flags f) { return f.less; });
		at(jle) = jump<jle>([](VirtualMachine::Flags f) { return f.less || f.equal; });
		at(jg)  = jump<jg> ([](VirtualMachine::Flags f) { return !f.less && !f.equal; });
		at(jge) = jump<jge>([](VirtualMachine::Flags f) { return !f.less; });
