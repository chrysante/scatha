...
template <typename T> T *allocateArrayUninit(MonotonicBufferAllocator &alloc, size_t count) {
    T *result = (T *)alloc.allocate(count * sizeof(T), alignof(T));
    return result;
}

template <typename T, typename Itr> std::span<T> allocateArray(MonotonicBufferAllocator &alloc, Itr begin, Itr end) {
