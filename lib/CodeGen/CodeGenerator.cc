...
static assembly::Label   toAsm(ic::Label const &l) { return assembly::Label(l.functionID.rawValue(), l.index); }

static assembly::Label   toAsm(ic::FunctionLabel const &l) { return assembly::Label(l.functionID().rawValue()); }
...
assembly::AssemblyStream CodeGenerator::run() {
    ...
    for (size_t index = 0; index < tac.statements.size(); ++index) {
        ...
        std::visit(utl::visitor{
                       [&](ic::Label const &label) { a << toAsm(label); },
...
void CodeGenerator::generateConditionalJump(assembly::AssemblyStream &a, ic::ThreeAddressStatement const &ifStatement,
...
void CodeGenerator::ResolvedArg::streamInsert(assembly::AssemblyStream &str) const {
    arg.visit(utl::visitor{[&](ic::EmptyArgument const &) { SC_DEBUGBREAK(); },
                           ...
                           [&](ic::Label const &label) { str << toAsm(label); }, [&](ic::If) { SC_DEBUGFAIL(); }});
}
...
CodeGenerator::ResolvedArg CodeGenerator::resolve(ic::TasArgument const &arg) { return {*this, arg}; }
