...
namespace scatha::ast {
	...
	auto constexpr DefaultCase(auto&& callback) {
		...
		return utl::visitor{
			[&](AlmostSameAs<TranslationUnit> auto&& tu) {
			...
			[&](AlmostSameAs<ReturnStatement> auto&& s) {
				callback(*s.expression);
			},
			...
			[&](AlmostSameAs<UnaryPrefixExpression> auto&& e) {
				callback(*e.operand);
			},
			...
			[&](AlmostSameAs<MemberAccess> auto&& m) {
				callback(*m.object);
			},
...
namespace scatha::ast::internal {
	decltype(auto) visitImpl(auto&& node, auto type, auto const& f) {
		switch (type) {
			...
			case NodeType::FloatingPointLiteral:  return f(static_cast<utl::copy_cvref_t<decltype(node), FloatingPointLiteral>>(node));
			case NodeType::StringLiteral:         return f(static_cast<utl::copy_cvref_t<decltype(node), StringLiteral>>(node));
			case NodeType::UnaryPrefixExpression: return f(static_cast<utl::copy_cvref_t<decltype(node), UnaryPrefixExpression>>(node));
