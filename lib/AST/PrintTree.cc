...
void                  printTree(AbstractSyntaxTree const *root) { printTree(root, std::cout); }
...
void                  printTree(AbstractSyntaxTree const *root, std::ostream &str) { printTreeImpl(root, str, 0); }
...
void printExpression(Expression const &expr) { printExpression(expr, std::cout); }

void printExpression(Expression const &expr, std::ostream &str) {
    visit(expr, utl::visitor{[&](Identifier const &id) { str << id.value(); },
