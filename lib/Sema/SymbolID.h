...
public:
    ...
    constexpr explicit SymbolID(u64 rawValue, SymbolCategory category):
        ...
        SC_EXPECT(rawValue < (u64(1) << 60),
                  "The SymbolID index must fit into the first 60 bits of the "
                  "structure.");
    }
    ...
    constexpr bool operator==(SymbolID const& rhs) const {
        ...
        SC_ASSERT(!result || _cat == rhs._cat,
                  "If values are equal then categories must also be equal.");
        return result;
    }
...
struct std::hash<scatha::sema::SymbolID> {
    std::size_t operator()(scatha::sema::SymbolID id) const {
        return std::hash<scatha::u64>{}(id.rawValue());
    }
};

template <>
struct std::hash<scatha::sema::TypeID> {
    std::size_t operator()(scatha::sema::TypeID id) const {
        return std::hash<scatha::sema::SymbolID>{}(id);
    }
};
