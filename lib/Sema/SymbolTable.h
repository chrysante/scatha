...
public:
    ...
    /// \returns \p InvalidDeclaration with reason \p Redefinition if declared name is already in use in the current scope.
    ...
    /// \details For successful return the name must not have been declared before in the current scope as some entity other than \p Function
    /// \returns Const reference to declared function if no error occurs.
    /// \returns \p InvalidDeclaration with reason \p Redefinition if declared name is already used by another kind of entity in the current scope.
    ...
    /// \details We need this two step way of addings functions to first scan all declarations to allow for forward references to other entities.
    /// \returns Nothing if  \p signature is a legal overload.
    /// \returns \p InvalidDeclaration with reason \p Redefinition if \p signature is not a legal overload,
    /// with reason \p CantOverloadOnReturnType if \p signature has same arguments as another function
    /// in the overload set but different return type .
