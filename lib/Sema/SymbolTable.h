...
class SCATHA(API) SymbolTable {
  public:
    ...
    Expected<ObjectType &, SemanticIssue>     addObjectType(Token name, size_t size = -1, size_t align = -1,
  ...
  private:
    ...
    // surrounding scopes.
    template <typename T> using EntitySet = utl::node_hashset<T, EntityBase::MapHash, EntityBase::MapEqual>;
