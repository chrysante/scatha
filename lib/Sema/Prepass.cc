...
/**
 * In prepass we declare all types including nested types in a first pass and
 * try to figure out their sizes and alignments. All functions and types of
 * which we can't determine the size because it has members of incomplete or
 * undeclared type will be pushed into a list. Then that list is repeatedly
 * traversed and successfully registered declarations will be removed from the
 * list until it is empty or its size does not change anymore. Then a last pass
 * over the list is run and we collect the appropriate errors.
 */
...
struct PrepassContext {
    explicit PrepassContext(SymbolTable &sym, issue::IssueHandler &iss): sym(sym), iss(iss) {}
...
SymbolTable prepass(ast::AbstractSyntaxTree &root, issue::IssueHandler &iss) {
    ...
    while (!ctx.unhandledStatements.empty()) {
        ...
        for (size_t i = 0; i < ctx.unhandledStatements.size();) {
            ...
            bool const success =
                sym.withScopeCurrent(x.enclosingScope, [&] { return ctx.dispatch(*x.statement); });
...
bool PrepassContext::prepass(ast::FunctionDefinition &fn) {
    ...
    for (auto &param : fn.parameters) {
        ...
        if (typeExprRes.category() != ast::EntityCategory::Type) {
            if (lastPass) {
                iss.push(BadSymbolReference(*param->typeExpr, param->typeExpr->category,
    ...
    if (returnTypeExprRes) {
        if (returnTypeExprRes.category() != ast::EntityCategory::Type) {
            if (lastPass) {
                iss.push(BadSymbolReference(*fn.returnTypeExpr, fn.returnTypeExpr->category,
                                            ast::EntityCategory::Type));
            } else {
                return false;
            }
    ...
    /// Declare the function. If we get here before the last bailout pass, check
    /// that our types are valid
    if (!lastPass) {
        if (!returnTypeID) {
            return false;
        }
    ...
    /// Might be TypeID::Invalid if we are in the last pass but we still declare
...
bool PrepassContext::prepass(ast::VariableDeclaration &decl) {
    ...
    SC_ASSERT(decl.typeExpr, "In structs variables need explicit type "
