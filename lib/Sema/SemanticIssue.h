...
class SCATHA(API) BadFunctionCall: public BadExpression {
  ...
  public:
    explicit BadFunctionCall(ast::Expression const &expression, SymbolID overloadSetID,
                             utl::small_vector<TypeID> argTypeIDs, Reason reason):
        BadExpression(expression),
        _reason(reason), _argTypeIDs(std::move(argTypeIDs)), _overloadSetID(overloadSetID) {}
...
class SCATHA(API) BadSymbolReference: public BadExpression {
  public:
    explicit BadSymbolReference(ast::Expression const &expression, ast::EntityCategory have,
                                ast::EntityCategory expected):
        BadExpression(expression),
        _have(have), _expected(expected) {}
...
class SCATHA(API) InvalidDeclaration: public InvalidStatement {
  ...
  public:
    explicit InvalidDeclaration(ast::Statement const *statement, Reason reason, Scope const &currentScope,
        ...
        _reason(reason), _category(symbolCategory), _existingCategory(existingSymbolCategory.value_or(symbolCategory)) {
    }
...
namespace internal {
using SemaIssueVariant =
    std::variant<BadTypeConversion, BadOperandForUnaryExpression, BadOperandsForBinaryExpression, BadMemberAccess,
                 BadFunctionCall, UseOfUndeclaredIdentifier, BadSymbolReference, InvalidStatement, InvalidDeclaration>;
}

class SCATHA(API) SemanticIssue: private internal::SemaIssueVariant {
  private:
    template <typename T> static decltype(auto) visitImpl(auto &&f, auto &&v) {
        return std::visit(utl::visitor{f,
  ...
  public:
    ...
    template <typename T = void> decltype(auto) visit(auto &&f) { return visitImpl<T>(f, asBase()); }

    template <typename T = void> decltype(auto) visit(auto &&f) const { return visitImpl<T>(f, asBase()); }

    template <typename T> auto                 &get() { return std::get<T>(asBase()); }

    template <typename T> auto const           &get() const { return std::get<T>(asBase()); }

    template <typename T> bool                  is() const { return std::holds_alternative<T>(asBase()); }
