...
class GraphNode {
    /// Return and pass small trivial types by value, everything else by `const&`.
    using PayloadView = std::conditional_t<(sizeof(Payload) <= 16) && std::is_trivially_copyable_v<Payload>,
                                           Payload,
                                           Payload const&>;
    
    /// To add const-ness to the pointee if payload is a pointer.
    using PayloadViewConstPtr = std::conditional_t<std::is_pointer_v<PayloadView>,
                                                   std::remove_pointer_t<PayloadView> const*,
                                                   PayloadView>;
    
    /// Either `GraphNode` or `Derived` is specified.
    using Self = std::conditional_t<std::is_same_v<Derived, void>, GraphNode, Derived>;
    
    /// For a tree node parent link is just a pointer, for a graph node it's a vector of pointers.
    using ParentLink = std::conditional_t<IsTree, Self*, utl::small_vector<Self*>>;
    
public:
    /// `PayloadHash` and `PayloadEqual` exist to put `GraphNode` in hashset where only the payload is used as the key.
    struct PayloadHash {
        ...
        size_t operator()(PayloadViewConstPtr payload) const {
            return std::hash<std::decay_t<PayloadViewConstPtr>>{}(static_cast<PayloadViewConstPtr>(payload));
        }
    };
    ...
    Self const& parent() const requires IsTree { return *_parentLink; }
    
    auto children() const requires IsTree {
        return outgoingImpl();
    }
...
private:
    auto outgoingImpl() const {
        return _outgoingEdges | ranges::views::transform([](auto* p) -> Self const& { return *p; });
    }
