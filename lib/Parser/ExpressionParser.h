...
/*
 ...
 and	| Left to right -> |					| remainder
 | 4		| +, -				| Addition and subtraction
 | 5		| <<, >>			| Bitwise left and right shift
 | 6		| <, <=, >, >=		| Relational operators | 7
...
/*
 ...
 <logical-or-expression> | <logical-or-expression> "?" <comma-expression> ":"
 ...
 "-" <unary-expression> | "~" <unary-expression> | "!" <unary-expression>
 <postfix-expression>			-->> <primary-expression>
                                                                   |
 <postfix-expression> "[" {<assignment-expression>}+ "]" | <postfix-expression>
 "(" {<assignment-expression>}* ")" | <postfix-expression> "." <identifier>
 <primary-expression> 			-->> <identifier>
                                                                   |
 <integer-literal> | <string-literal> | "(" <comma-expression> ")"
...
class SCATHA(API) ExpressionParser {
  ...
  private:
    ...
    ast::UniquePtr<Expr>           parseFunctionCallLike(ast::UniquePtr<ast::Expression> primary, std::string_view open,
    ...
    template <ast::BinaryOperator...> ast::UniquePtr<ast::Expression> parseBinaryOperatorLTR(auto &&operand);

    template <ast::BinaryOperator...> ast::UniquePtr<ast::Expression> parseBinaryOperatorRTL(auto &&parseOperand);
