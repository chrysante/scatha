...
public:
    ...
    explicit StructureType(std::string name, std::span<Type const* const> members): Type(std::move(name), Type::Category::Structure), _members(members) {}
    
    Type const* memberAt(std::size_t index) const { return _members[index]; }
    
    std::span<Type const* const> members() const {
        return _members;
    }
